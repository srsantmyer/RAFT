...
public class DriverScript {
	...
	private void setDefaultTestParameters() {
		...
		if (testParameters.getRecordVideo() == null) {
			testParameters.setRecordVideo(properties.getProperty("RecordVideo"));
		}
	...
	private void initializeWebDriver() {
		...
		switch (testParameters.getExecutionMode()) {
			...
			case LOCAL:
				...
				if (isHeadless || testParameters.getBrowser().name().contains("HEADLESS")) {
			...
			case GRID:
				try {
					WebDriver remoteGridDriver = WebDriverFactory.getRemoteWebDriver(testParameters.getBrowser(),
							...
							testParameters.getRecordVideo(),
							testParameters.getTimezone(),
							testParameters.getScreenResolution(),
					...
					var m = ex.getMessage();
					m = m.toString();
					// send some chrome instead
					WebDriver webDriverC = WebDriverFactory.getWebDriver(testParameters.getBrowser());
					driver = new CustomDriver(webDriverC);
					driver.setTestParameters(testParameters);
					if (isHeadless || testParameters.getBrowser().name().contains("HEADLESS")) {
						driver.manage().window().setSize(new Dimension(1920, 1080));
					} else {
